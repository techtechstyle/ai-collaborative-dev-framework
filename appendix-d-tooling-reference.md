...
ガイドの4つの哲学(持続可能性、心理的安全性、急がば回れ、シンプルさ)は階層構造を持ち、上位Levelが常に優先されます。このツールは、判断が必要な場面で階層に沿った一貫した判断を支援します。
...
このガイドの例示はTypeScript/NestJS/zodに偏っていますが、**原則は言語・フレームワークに依存しません**。
...
| **Unionåž‹** | `type Role = 'admin' \| 'user'` | `Literal['admin', 'user']` | `enum class Role` | `enum Role` |
