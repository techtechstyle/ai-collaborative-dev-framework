...
v4.7では、「増強の哲学」に**双方向性の視点**を追加し、各哲学層が防いでいる「負の増幅」を明示しました:

| 追加内容 | 説明 |
|---------|------|
| **増強メカニズムの双方向性** | 増強は正も負も等しく増幅される。各層が特定の「負の増幅」を防ぐ防御構造 |
| **各哲学層の「防いでいる負の増幅」** | L0:依存の自己強化、L1:エラー隠蔽、L2:エラーの連鎖的蓄積、L3:複雑性の自己増殖、L4:認知資源の浪費 |
| **「べき論」から「構造的必然」への転換** | 各概念の存在理由を「防御」として説明 |
| **L4の特殊性の明確化** | L4は唯一「他の層による抑制を必要とする層」 |

> 📖 詳細は [00b-augmentation.md](./00b-augmentation.md)、各哲学レイヤーファイル（00c〜00g）を参照

#### v4.6で追加済みの概念

v4.6では、「1文1検証」原則に**反証（falsification）的思考**の視点を追加しました:

| 追加内容 | 説明 |
|---------|------|
| **反証的構造の導入** | Dijkstra（1969）とPopperの反証主義の構造的同型性を解説 |
| **検証と反証の認識論的違い** | 「正しいことを確認する」と「間違いを見つけようとする」の根本的差異 |
| **フレームワーク内の反証マップ** | 既存要素（フェイルファスト、F3、ポカヨケ、損切りルール等）を「反証」で統一的に再解釈 |
...
| バージョン | 主な変更 |
|-----------|----------|
| v4.7 | 「増強の哲学」に双方向性の視点追加。各哲学層が防ぐ「負の増幅」を明示、「べき論」→「構造的必然」への転換 |
| v4.6 | 「1文1検証」に反証（falsification）的思考を追加。Dijkstra/Popperの構造的同型性、反証マップ、AI反証プロンプト |
...
| Dijkstra, "Notes on Structured Programming" (1970) | Part 0 反証的構造 | 「テストはバグの存在を示すが、不在は示せない」 |
...
| Dave Farley『Modern Software Engineering』2021 | Part 0 反証的構造、科学的アプローチとしてのソフトウェア工学 |
