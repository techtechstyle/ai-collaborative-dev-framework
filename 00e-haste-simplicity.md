...
### この層が防いでいる負の増幅:エラーの連鎖的蓄積

増強のメカニズムは双方向に作用します（[増強の双方向性](./00b-augmentation.md)参照）。

L2が防いでいるのは**「エラーの連鎖的蓄積」**という負の増幅です。

```
検証を省略する → 小さなエラーが残る → AIが次のステップでそのエラーを前提にする
                                          → エラーが雪だるま式に増幅される
```

AIは人間と異なり、「何かおかしい」という直感を持ちません。
前のステップに小さなエラーがあっても、AIはそれを前提として忠実に次の出力を生成します。
検証を省略するたびに、**増幅される前にエラーを止める機会を一つ失います。**

**「1文1検証」は、増幅される前にエラーを止める最小単位の防御です。**
「丁寧にやりましょう」という精神論ではなく、
増強のメカニズムが裏返るのを各ステップで食い止める構造的な仕組みです。
...
```mermaid
flowchart TD
    subgraph Want["やりたいこと → 現実の制約"]
        W1["レビューを増やしたい → 工数が増える"]
        W2["テストを厚くしたい → 実装時間が減る"]
        W3["設計を練りたい → 着手が遅れる"]
        W4["ドキュメントを整備したい → 誰も読まない"]
        W5["振り返りをしたい → 次の案件が待っている"]
    end

    Want --> Pressure["「とりあえず動くものを」という圧力に負ける"]
    Pressure --> Debt["技術的負債の蓄積 → 後で高いコストを払う"]

    style Want fill:#fff3e0,stroke:#e65100
    style Pressure fill:#fce4ec,stroke:#c62828
    style Debt fill:#ffcdd2,stroke:#b71c1c
```
...
```mermaid
flowchart TD
    A["1回目の検証で失敗"] -->|"即座に報告"| B["typecheckでエラーが発生しました。<br>エラー内容: ［具体的なエラー］<br>修正が必要です。"]
    B -->|"原因分析"| C["このエラーは○○が原因です。<br>解決策: △△を修正します。"]
    C -->|"修正 & 再検証"| D["修正完了 → 再度検証"]
```
...
```mermaid
flowchart TD
    A["3往復経過"] -->|"即座に停止"| B["3往復しましたが解決していません。<br>現状: ［具体的な状態］"]
    B --> C{"アプローチを変更"}
    C --> D["選択肢A: ［代替案1］"]
    C --> E["選択肢B: ［代替案2］"]
```
...
```mermaid
flowchart TB
    L1["<b>Level 1: 防止（Prevention）</b><br>そもそも間違いが起きない設計<br>例: 型システム、必須パラメータ、enum制約"]
    L2["<b>Level 2: 検出（Detection）</b><br>間違いが起きたら即座に検出<br>例: typecheck, lint, 自動テスト"]
    L3["<b>Level 3: 緩和（Mitigation）</b><br>間違いの影響を最小限に抑える<br>例: ロールバック、可逆性設計"]
...
>
> **補足**: この「シンプルさ」は、禅の美学における「簡素（kanso）」— 不要なものを削ぎ落とし本質を残す精神 — にも通じます。ただし、本フレームワークでは「引き算の美」だけでなく、「分解して管理可能にする」という工学的な意味合いも含みます。

### この層が防いでいる負の増幅:複雑性の自己増殖

増強のメカニズムは双方向に作用します（[増強の双方向性](./00b-augmentation.md)参照）。

L3が防いでいるのは**「複雑性の自己増殖」**という負の増幅です。

```
曖昧な指示 → AIが解釈を補完して複雑な出力 → 人間が把握しきれない
    ↑                                                         ↓
    └────────── さらに曖昧な修正指示 ←────────────────────────┘
```

AIは曖昧さを受け取ると、「親切心」から解釈を補完して出力します。
その出力は往々にして入力より複雑になり、人間が全体を把握できなくなります。
把握できないまま修正指示を出すと、さらに複雑な出力が返る。
**増強のメカニズムが、複雑性を自己増殖させる装置になります。**

**シンプルさの哲学は、増幅される入力そのものを制御する設計です。**
「1文で説明でき、検証1回で確認できる」粒度にすることで、
AIに渡す入力の品質を構造的に保証し、複雑性の自己増殖を断ち切ります。
...
```mermaid
flowchart TD
    A[タスクを受け取る] --> B{"1文で説明<br>できるか?"}
    B -- No --> D1["分解が必要<br>（何を、をより具体的に）"]
    B -- Yes --> C{"検証1回で<br>完了確認?"}
    C -- No --> D2["分解が必要<br>（成果物ごとに分割）"]
    C -- Yes --> E["AIに渡せる ✅"]
```
