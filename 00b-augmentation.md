...
このフレームワークの根底には、**AI協働開発の本質は「人間の置き換え」ではなく「人間の増強」である**という哲学があります。
...
**4. 増強のメカニズムは双方向に作用する — 正も負も等しく増幅される**

これは増強の哲学において最も誠実に向き合うべき事実です。

AIは人間が与えたものを増幅します。明確な指示を与えれば、明確で正確な出力が返る。
...
│  ─────────────────                                              │
│  ・曖昧な指示 → 見当違いな出力の量産                            │
│  ・誤った前提 → エラーの上にエラーを積み重ねる                  │
│  ・検証の省略 → 問題が雪だるま式に拡大                          │
│  ・間違った教訓の記録 → 偏見のスケーラブルな適用                │
│                                                                 │
│  増幅のメカニズムには方向性がない。                             │
│  正しいものも間違ったものも、等しく増幅される。                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

LLMは「正誤に関わらず均一な確信度で生成する」という特性を持っています。
人間のエラーを受け取ったとき、エラーであることを指摘せずに増幅する可能性が高い。
これは増強のメカニズムがそのまま裏返る構造です。

**だからこそ、このフレームワークの各層（L0〜L4）が必要になります。**

各層は単なる「べき論」ではありません。
増強が裏返ったときに、具体的にどの負の増幅を止めるかを定義した**防御構造**です。

### 増強の哲学と階層構造の関係 — 各層が防いでいる負の増幅
...
| Level | 哲学 | 増強の哲学との関係 | 防いでいる負の増幅 |
|-------|------|------------------|------------------|
| L0 | 持続可能性 | 人間が成長し続けるから持続可能 | **依存の自己強化** — AIに頼る→スキル衰退→さらに頼るループ |
| L1 | 心理的安全性 | 人間が制御しているから安心 | **エラー隠蔽の増幅** — 恐れ→隠す→AIが隠れた問題を拡大 |
| L2 | 急がば回れ | 人間が検証するから丁寧に進める | **エラーの連鎖的蓄積** — 検証省略→小さなエラー→雪だるま式に増幅 |
| L3 | シンプルさ | 人間が理解できる粒度に分解 | **複雑性の自己増殖** — 曖昧な入力→複雑な出力→さらに曖昧な修正 |
| L4 | AIファースト | AIの強みを積極活用 | **認知資源の浪費** — ただし単独では成長機会の喪失を招く（※） |

> ※ L4は唯一、**他の層による抑制を必要とする層**です。L0の可逆性、L2のティーチングモード、
> L0の持続可能性が常に制御していなければ安全に機能しません。
> これが、L4が階層の最下位に置かれている理由の一つです。

> **参考**: Anthropicは「人間によるAI監視の支援」をClaudeの最優先事項として設計しています。
> このフレームワークの「増強の哲学」は、この設計思想と共鳴しています。
...
このフレームワークでAIを「パートナー」と呼ぶのは、対等だからではありません。
