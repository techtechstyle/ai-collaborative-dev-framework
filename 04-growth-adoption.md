...
│  ────────────────────────        ──────────────────              │
│  ・理解せずに使う                ・特性と限界を学ぶ              │
│  ・うまくいかない→AIのせい      ・うまくいかない→人間も反省    │
│  ・すぐに諦める                  ・改善方法を探る                │
│  ・人間のスキルは停滞            ・人間も成長する                │
│                                                                 │
│  → 「パートナー」とは対等という意味ではない                     │
│  → 「理解し、真剣に向き合う」という姿勢のこと                   │
└─────────────────────────────────────────────────────────────────┘
```
...
> 💡 背景理論(SECIモデル)の詳細は [付録C](./appendix-c-theory.md#c2-seciモデル知識の変換プロセス) を参照。
...
```
Claudeの成長サイクル:
  実装 → 検証 → エラー発見 → CLAUDE.mdに追記 → 次回改善
  ✅ 仕組みとして組み込まれている

人間の成長サイクル(意識しないと):
  指示 → Claudeが実装 → 完成 → ... 次の指示
  ❓ 学びを得る機会がない
